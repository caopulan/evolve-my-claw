...
export function defaultIdentityPath(stateDir?: string): string {
  const root = stateDir?.trim() || path.join(os.homedir(), ".openclaw");
  return path.join(root, "identity", "device.json");
}
